import { loginUser, issueSessionToken, entitlement } from "@/app/_lib/auth";
...
export async function POST(req: Request) {
  try {
    ...
    return privateJson({ ok: true, token: await issueSessionToken(r.user.id), ...entitlement(r.user) });
  } catch {
