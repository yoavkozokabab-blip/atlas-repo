...
as $$
  ...
  select jsonb_build_object(
    ...
    'successful_installs', count(distinct installation_id) filter (where event_name = 'desktop_installed'),
    'first_launches', count(*) filter (where event_name = 'desktop_launched'),
    'active_installations', count(distinct installation_id) filter (where event_name = 'desktop_launched'),
    'scans_completed', count(*) filter (where event_name in ('sample_scan_completed', 'real_repo_scan_completed')),
    'graphs_opened', count(*) filter (where event_name = 'graph_opened'),
    'impact_completed', count(*) filter (where event_name = 'impact_completed'),
    'agents_connected', count(*) filter (where event_name = 'mcp_connected'),
    'signup_success', count(*) filter (where event_name = 'account_create_success'),
    'signup_failed', count(*) filter (where event_name = 'account_create_failed'),
    'login_success', count(*) filter (where event_name = 'login_success'),
