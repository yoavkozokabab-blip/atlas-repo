...
MCP_AGENT_DEFINITIONS: Tuple[Dict[str, Any], ...] = (
    {
        "id": "cursor",
        "label": "Cursor",
        "connect_label": "Connect to Cursor",
        "description": "One-click MCP setup via ~/.cursor/mcp.json",
        "availability": "available",
        "config_format": "json",
        "restart_hint": "Restart Cursor, then open Settings → MCP and confirm Atlas is connected.",
    },
    {
        "id": "claude",
        "label": "Claude",
        "connect_label": "Connect to Claude",
        "description": "One-click MCP setup via Claude Desktop config (mcpServers)",
        "availability": "available",
        "config_format": "json",
        "restart_hint": "Restart Claude Desktop, then confirm Atlas appears in MCP settings.",
    },
    {
        "id": "codex",
        "label": "Codex",
        "connect_label": "Connect to Codex",
        "description": "One-click MCP setup via ~/.codex/config.toml ([mcp_servers.atlas])",
        "availability": "experimental",
        "config_format": "toml",
        "restart_hint": "Restart the Codex app or IDE extension, then confirm Atlas under MCP settings.",
    },
)
...
def write_cursor_config(*, confirm: bool = False, server_name: str = "atlas") -> Dict[str, Any]:
    ...
    if result.get("ok"):
        ...
        try:
            ...
            _usage.track_agent_connected("cursor")
        except Exception:
            try:
                from . import cloud_analytics as _cloud
                _cloud.track_cloud_event("agent_context_injected", metadata={"agent": "cursor"})
            except Exception:
                pass
    return result
...
def write_claude_config(*, confirm: bool = False, server_name: str = "atlas") -> Dict[str, Any]:
    ...
    if result.get("ok"):
        ...
        try:
            from . import usage_analytics as _usage
            _usage.track_agent_connected("claude")
        except Exception:
            try:
                from . import cloud_analytics as _cloud
                _cloud.track_cloud_event("agent_context_injected", metadata={"agent": "claude"})
            except Exception:
                pass
    return result
...
def write_codex_config(*, confirm: bool = False, server_name: str = "atlas") -> Dict[str, Any]:
    ...
    try:
        from . import usage_analytics as _usage
        _usage.track_agent_connected("codex")
    except Exception:
        try:
            from . import cloud_analytics as _cloud
            _cloud.track_cloud_event("agent_context_injected", metadata={"agent": "codex"})
        except Exception:
            pass
    return {
        "ok": True,
        ...
        "message": (
            ...
            "then confirm Atlas appears under MCP settings."
        ),
    }


def _agent_status_handlers() -> Dict[str, Callable[[], Dict[str, Any]]]:
    return {
...
def _agent_write_handlers() -> Dict[str, Callable[..., Dict[str, Any]]]:
    return {
...
def list_mcp_agent_clients() -> List[Dict[str, Any]]:
    handlers = _agent_status_handlers()
    out: List[Dict[str, Any]] = []
    for defn in MCP_AGENT_DEFINITIONS:
        agent_id = str(defn["id"])
        status = handlers[agent_id]()
        out.append(
            {
                **dict(defn),
                "connected": bool(status.get("atlas_configured")),
                "config_path": status.get("config_path"),
                "can_auto_write": status.get("can_auto_write", True),
                "status": status,
            }
        )
    return out
...
def write_agent_config(agent_id: str, *, confirm: bool = False, server_name: str = "atlas") -> Dict[str, Any]:
    handler = _agent_write_handlers().get(str(agent_id or "").strip().lower())
    if handler is None:
        return {"ok": False, "code": "unknown_agent", "error": f"Unknown MCP agent: {agent_id}"}
    return handler(confirm=confirm, server_name=server_name)
