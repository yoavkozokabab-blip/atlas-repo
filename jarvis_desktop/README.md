...
From the `local_jarvis` directory:
...
```bash
py run_jarvis_desktop.py --port 8777 --host 127.0.0.1 --no-browser
```
...
1. **Read onboarding** — explains what JARVIS does (shown once).
...
**Try Demo Mode** loads a bundled mini repository (`jarvis_desktop/demo/sample_repo/`) without choosing a path. The header shows a **Demo Mode** badge. All features work the same; data is labeled as demo.
...
- `jarvis_desktop/api.py` — product logic (scan, graph, copilot, validation)
- `jarvis_desktop/server.py` — stdlib HTTP server + optional FastAPI
- `jarvis_desktop/static/` — SPA (HTML/CSS/JS)
