...
def test_showcase_readme_exists_and_complete():
    path = os.path.join(ROOT, "README_JARVIS.md")
    assert os.path.isfile(path)
    md = open(path, "r", encoding="utf-8").read()
    assert "Stop making AI read your entire repository" in md       # value prop < 60s
    for section in ("What is JARVIS", "Why JARVIS exists", "Screenshots", "Demo",
                    "Architecture", "Features", "Roadmap", "Beta access"):
        assert section in md, f"README missing section: {section}"
