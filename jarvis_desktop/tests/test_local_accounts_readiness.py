...
def test_service_running_requires_invite_validation_probe():
    ...
    with patch("jarvis_desktop.accounts_client.auth_mode", return_value="local"):
        with patch("jarvis_desktop.accounts_client._call", side_effect=broken_call):
    ...
    with patch("jarvis_desktop.accounts_client.auth_mode", return_value="local"):
        with patch("jarvis_desktop.accounts_client._call", side_effect=ready_call):
...
def test_backend_500_is_not_shown_as_registration_error():
    with patch("jarvis_desktop.accounts_service_runner.ensure_running", return_value=True):
        with patch(
            "jarvis_desktop.accounts_client.register",
