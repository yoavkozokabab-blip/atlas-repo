...
class TestNodeVisibilityContract:
    def test_global_three_script_present(self):
        ...
        assert "three@0.157.0/build/three.min.js" in INDEX
    ...
    def test_risk_and_cycle_coloring(self):
        ...
        assert "0x9a7bff" in block          # purple
        assert "0xff5c7a" in block          # high risk red/pink
        assert "0xffc24b" in block          # elevated amber
    ...
    def test_fog_reduced(self):
        # Heavy fog hid the graph; Phase 124 uses a very light density.
        assert "FogExp2(0x070b14, 0.00035)" in UNIVERSE
...
class TestCleanLayout:
    ...
    def test_advanced_toolbar_groups_decorative_actions(self):
        # Export bundle / PNG / SVG / Screenshot / Tour live under Advanced.
        adv = INDEX[INDEX.index("graph-advanced-toolbar"):]
        adv = adv[: adv.index("</details>")]
        for action in ("exportDemoBundle", "exportGraphPNG", "exportGraphSVG",
    ...
    def test_concise_warning_with_details(self):
        assert "Graph coverage is partial. Most missing links are external or dynamic imports." in APP
...
class TestModuleListAndInspector:
    ...
    def test_module_list_has_required_columns(self):
        ...
        assert "in ${fi}" in block and "out ${fo}" in block
