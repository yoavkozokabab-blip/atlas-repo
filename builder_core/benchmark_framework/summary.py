"""Codex Alone vs Atlas + Codex aggregate reporting."""
...
def aggregate(run_logs: list[RunLog], manual_scores: list[ManualScore]) -> Dict[str, Any]:
    ...
    wins = {"atlas_plus_codex": 0, "codex_alone": 0, "tie": 0, "incomplete": 0}

    for task_id in task_ids:
        base_log = logs.get((task_id, "codex_alone"))
        atlas_log = logs.get((task_id, "atlas_plus_codex"))
        base_score = scores.get((task_id, "codex_alone"))
        atlas_score = scores.get((task_id, "atlas_plus_codex"))
        row: Dict[str, Any] = {"task_id": task_id}
        if base_log:
            row["codex_alone"] = {"run_log": base_log.to_dict()}
        if atlas_log:
            row["atlas_plus_codex"] = {"run_log": atlas_log.to_dict()}
        if base_score:
            row.setdefault("codex_alone", {})["score"] = base_score.to_dict()
        if atlas_score:
            row.setdefault("atlas_plus_codex", {})["score"] = atlas_score.to_dict()

        if not all((base_log, atlas_log, base_score, atlas_score)):
            row["winner"] = "incomplete"
            wins["incomplete"] += 1
            per_task.append(row)
            continue

        reduction = _percent_reduction(base_log.estimated_total_tokens, atlas_log.estimated_total_tokens)
        speedup = _ratio(base_log.elapsed_seconds, atlas_log.elapsed_seconds)
        delta = round(atlas_score.total_score - base_score.total_score, 2)
        ...
        if delta > 0:
            winner = "atlas_plus_codex"
        elif delta < 0:
            winner = "codex_alone"
        elif atlas_score.task_success and not base_score.task_success:
            winner = "atlas_plus_codex"
        elif base_score.task_success and not atlas_score.task_success:
            winner = "codex_alone"
        else:
            winner = "tie"
        row["winner"] = winner
    ...
    return {
        "comparison": "Codex Alone vs Atlas + Codex",
...
def render_markdown(summary: Dict[str, Any], *, run_id: str) -> str:
    ...
    atlas = summary["modes"]["atlas_plus_codex"]
    outcomes = summary["win_loss_tie"]
    lines = [
        f"# Benchmark Summary: Codex Alone vs Atlas + Codex ({run_id})",
        "",
        "> All token numbers are estimates unless manually overridden.",
        "",
        "## Aggregate",
        "",
        "| Metric | Codex Alone | Atlas + Codex |",
        "|---|---:|---:|",
        f"| Logged runs | {baseline['runs_logged']} | {atlas['runs_logged']} |",
        f"| Scored runs | {baseline['runs_scored']} | {atlas['runs_scored']} |",
        f"| Average estimated input tokens | {_display(baseline['average_estimated_input_tokens'])} | {_display(atlas['average_estimated_input_tokens'])} |",
        f"| Average estimated output tokens | {_display(baseline['average_estimated_output_tokens'])} | {_display(atlas['average_estimated_output_tokens'])} |",
        f"| Average estimated total tokens | {_display(baseline['average_estimated_total_tokens'])} | {_display(atlas['average_estimated_total_tokens'])} |",
        f"| Average latency seconds | {_display(baseline['average_latency_seconds'])} | {_display(atlas['average_latency_seconds'])} |",
        f"| Task success rate | {_display(baseline['task_success_rate'])} | {_display(atlas['task_success_rate'])} |",
        f"| Correctness (0..5) | {_display(baseline['average_correctness'])} | {_display(atlas['average_correctness'])} |",
        f"| Evidence quality (0..5) | {_display(baseline['average_evidence_quality'])} | {_display(atlas['average_evidence_quality'])} |",
        f"| Answer completeness (0..5) | {_display(baseline['average_completeness'])} | {_display(atlas['average_completeness'])} |",
        f"| Hallucination risk (0..5, lower is better) | {_display(baseline['average_hallucination_risk'])} | {_display(atlas['average_hallucination_risk'])} |",
        f"| Average quality score | {_display(baseline['average_total_score'])} | {_display(atlas['average_total_score'])} |",
        "",
        ...
        f"- Win/loss/tie/incomplete: {outcomes['atlas_plus_codex']}/{outcomes['codex_alone']}/{outcomes['tie']}/{outcomes['incomplete']}",
        "",
        "## Bug-Finding Accuracy",
        "",
        "| Metric | Codex Alone | Atlas + Codex |",
        "|---|---:|---:|",
        f"| True positives | {baseline['bug_finding_accuracy']['true_positives']} | {atlas['bug_finding_accuracy']['true_positives']} |",
        f"| False positives | {baseline['bug_finding_accuracy']['false_positives']} | {atlas['bug_finding_accuracy']['false_positives']} |",
        f"| False negatives | {baseline['bug_finding_accuracy']['false_negatives']} | {atlas['bug_finding_accuracy']['false_negatives']} |",
        f"| Precision | {_display(baseline['bug_finding_accuracy']['precision'])} | {_display(atlas['bug_finding_accuracy']['precision'])} |",
        f"| Recall | {_display(baseline['bug_finding_accuracy']['recall'])} | {_display(atlas['bug_finding_accuracy']['recall'])} |",
        "",
        "## Per-task Breakdown",
        "",
