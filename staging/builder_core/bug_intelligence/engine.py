...
# Additive, findings-free fact augmenters. Remove entries from this list to
# disable Phase 93A / 96A with no finding behavior change.
_fact_augmenters = [A.InterproceduralAgent(), A.ContractFactsAgent()]
...
def analyze_source(
    ...
    *,
    ...
    from . import contract_enrichment
    findings = contract_enrichment.enrich_findings(findings, module_facts)

    from . import verification_evidence
    findings = verification_evidence.enrich_findings(
        findings, module_facts, test_documents=test_documents
    )

    from . import confirmed_defect_gate
    findings = confirmed_defect_gate.apply_gate(findings)
