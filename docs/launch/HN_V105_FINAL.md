# Show HN Package — Atlas v1.0.5 (Final)
...
plan is planned but there's nothing to buy yet. Anonymous usage analytics can be
turned off in Settings and never includes code, paths, prompts, or answers.
[Drop this sentence if analytics ships default-off or disabled.]
...
checks and (if enabled) anonymous analytics; analytics never includes code, paths,
...
From Atlas: update checks and (if enabled) anonymous analytics events — never
...
it off. [If default-off: "It's off by default; opt-in only."]
...
plainly. [Adjust to actual licensing state at post time.]
