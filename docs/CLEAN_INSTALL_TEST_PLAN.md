...
> spare machine). Run it once before inviting beta users and record PASS/FAIL per step.

## Prerequisites
- A clean Windows 10/11 machine or VM (no prior Atlas install).
- The **rebuilt** installer published to GitHub Releases (the 06-15 build is stale).
...
| 6 | Launch from Start Menu (or the post-install checkbox) | browser opens to the Atlas UI on `127.0.0.1:8777`; no error page | |
...
| 12 | Add/Remove Programs | shows **Atlas 0.1.0-beta** | |
...
## Pass criteria for limited beta
- Steps 1–10, 12–14 PASS.
- Step 11 (MCP) may remain **experimental** if not green — document, don't block.
- If step 14 leaves data behind, the uninstall fix didn't make it into the build → rebuild.
