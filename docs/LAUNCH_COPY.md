...
> Honest constraints baked in: retrieval is measured, outcomes are not; Windows only;
...
quality — outcome studies are next. Windows today.
...
Local-first; your code stays on your machine. Windows today.
...
fewer tokens. Windows download available. [link]

## Reddit post (r/ChatGPTCoding)
**Built a local context layer for AI agents that works on big repos.** Short problem, a
2-min demo + benchmark (including where it loses), Windows app, feedback welcome.
...
what's NOT done (Windows-only, unsigned app), and a request for feedback.
...
engine, switch from Claude to Cursor. (1:50) "local — your code stays here. Download Atlas,
link below."
