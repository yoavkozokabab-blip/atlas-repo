...
function Write-VersionResource {
    # Atlas.exe previously shipped with no Windows version resource at all:
    # right-click -> Properties -> Details was blank and FileVersion read
    # 0.0.0.0. Only the installer carried an identity. That costs trust on an
    # already-unsigned binary, so the executable now describes itself.
    $version = "1.0.0"
    $productFile = Join-Path $Root "atlas_desktop\product_info.py"
    if (Test-Path $productFile) {
        $m = Select-String -Path $productFile -Pattern 'PRODUCT_VERSION\s*=\s*"([^"]+)"' | Select-Object -First 1
    ...
    # VS_FIXEDFILEINFO takes four integers; a prerelease suffix is carried in
    # the string fields, which are what a user actually reads.
    $nums = @("1", "0", "0", "0")
    ...
    $content = @"
# Auto-generated by packaging/pyinstaller/build_atlas_exe.ps1  -  do not edit.
VSVersionInfo(
  ffi=FixedFileInfo(
    filevers=($tuple),
    prodvers=($tuple),
  ...
  kids=[
    StringFileInfo([
      StringTable('040904B0', [
        ...
        StringStruct('FileDescription', 'Atlas: local repository memory for AI coding agents'),
...
"@
    $out = Join-Path $ScriptDir "version_info.txt"
    $utf8 = New-Object System.Text.UTF8Encoding $false
    [System.IO.File]::WriteAllText($out, $content, $utf8)
    Write-Host "Version resource: $out ($version)" -ForegroundColor DarkGray
}
