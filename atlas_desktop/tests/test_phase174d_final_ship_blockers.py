...
from atlas_desktop.install_support import _redact_support_text, data_dir
...
def test_kubernetes_build_generic_prompt_refuses():
    ...
    with patch("atlas_desktop.api._planning_context", return_value={}), patch(
...
def test_kubernetes_investigation_generic_prompt_refuses():
    ...
    api._STATE.update(_k8s_state())
    with patch("atlas_desktop.api._planning_context", return_value={}), patch(
        "atlas_desktop.planning_engine.investigate_symptom",
        return_value={
            "ok": True,
            "plan": {
...
def test_support_bundle_does_not_contain_leak_me(tmp_path, monkeypatch):
    monkeypatch.setattr(
        "atlas_desktop.install_support.data_dir",
