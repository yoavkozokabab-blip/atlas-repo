...
from atlas_desktop import context_pack as cp
...
def test_ownership_penalty_targets_nonsource_dirs():
    ...
    assert cp._ownership_penalty("atlas_desktop/api.py")[0] == 0
    assert cp._ownership_penalty("websites/atlas-web/app/_lib/auth.ts")[0] == 0
