...
def _check_imports() -> Dict[str, Any]:
    missing: List[str] = []
    for mod in ("atlas_desktop", "atlas_desktop.api", "atlas_desktop.server"):
        try:
            __import__(mod)
    ...
    return {
        ...
        "hint": None if ok else "Reinstall Atlas or run from the folder that contains atlas_desktop. Do not pip install the monorepo requirements.txt.",
    }
