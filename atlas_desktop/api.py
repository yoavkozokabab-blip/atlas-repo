"""Framework-agnostic product API core for Atlas Desktop.
...
def _scan_repository_locked(path: Optional[str] = None, scope: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    build_meta = graph.get("atlas_graph_build") or {}
    ...
    recorder.mark(
        ...
        graph_timed_out=bool(graph.get("atlas_timed_out")),
        graph_partial=bool(graph.get("atlas_partial")),
    )
    if _STATE["scan_job"].get("cancelled"):
        return {"ok": False, "error": "Scan cancelled.", "code": "scan_cancelled"}
...
def build_full_module_graph() -> Dict[str, Any]:
    ...
    return {
        ...
        "degraded": bool(graph.get("degraded")),
        "atlas_timed_out": bool(graph.get("atlas_timed_out")),
        "elapsed_sec": elapsed,
...
def export_demo_bundle() -> Dict[str, Any]:
    ...
    with zipfile.ZipFile(buffer, "w", zipfile.ZIP_DEFLATED) as archive:
        ...
        archive.writestr(
            "screenshots/README.txt",
            "Capture marketing PNGs from Atlas Desktop Screenshot mode, then add them here.\n",
        )
        archive.writestr("landing_assets/checklists.md", landing_checklist)
    payload = buffer.getvalue()
    stamp = time.strftime("%Y%m%d_%H%M%S")
    filename = f"atlas_demo_bundle_{stamp}.zip"
...
def _answer_context_export(question: str, packet: str) -> Dict[str, Any]:
    ...
    answer = (
        f"Generated a {packet} Atlas context packet (~{tokens} tokens) for {primary.title()}. "
